...
def test_sse_initial_snapshot_is_tenant_scoped():
    async def scenario():
        ...
        return chunks, learner_a, learner_b

    chunks, learner_a, learner_b = asyncio.run(scenario())
...
def test_sse_emits_again_only_after_state_changes():
    async def scenario():
        projection = LearnerSupervisionProjection()
        learner_id = uuid4()
        projection.apply(EducationLearningEvent(
